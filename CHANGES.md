...
namespace with `beicon.v2` namespace, so the people really continue
using the old API and the old package if they want without a conflict
...
- The internal `rx/pipe` function inverts the arguments order for to
  be easy to use with `->>`.
- Remove the `flat-map` alias for `merge`.
...
- Replace `dedupe` and `dedupe'` with operator only
  `distinct-until-changed` and `distinct` for make it more
  similar to rxjs API.
